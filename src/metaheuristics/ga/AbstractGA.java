...
public abstract class AbstractGA<G extends Number, F> {
	...
	/**
	 * start time of execution; used to check if the limit time has been reached.
	 */
	protected long startTime;

	/**
	 * limit time of execution.
	 */
	...
	/**
	 * Check if the execution time reached the limit.
	 *
	...
	public boolean checkLimitTime() {
		...
		if ( (double)totalTime/(double)1000 > limitTime) {
			System.out.println("Limit time reached... stopping execution");
			return true;
		}
		return false;
	}
	 ...
	 */
	protected Population crossover(Population parents, int numberOfParents) {

		Population offsprings = new Population();

		for (int i = 0; i < numberOfParents; i = i + 2) {
